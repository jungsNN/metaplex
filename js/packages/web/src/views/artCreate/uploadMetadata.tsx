...
export const UploadMetadataView = () => {
  ...
  const gotoStep = useCallback(
    (_step: number) => {
      history.push(`/upload/${_step.toString()}`);
      ...
      <Row justify={width < 768 ? 'center' : 'start'}>
        <Col>
            <Row>
                ...
                className="type-btn"
                size="large"
                onClick={() => props.confirm((IMetadataCategory.XLS as unknown ) as MetadataCategory)}
                >
                <div>
                    <div>Metadata</div>
                    <div className="type-btn-description">XLS</div>
                </div>
                </Button>
            </Row>
          {/* <Row>
...
const UploadStep = (props: {
  ...
  const uploadMsg = (category: MetadataCategory | IMetadataCategory) => {
    switch (category) {
      ...
      case IMetadataCategory.XLS:
        return 'Upload your Excel(XLS) file(s)';
      default:
        return 'Please go back and choose a category';
    }
  };

  const acceptableFiles = (category: MetadataCategory | IMetadataCategory) => {
    switch (category) {
      case MetadataCategory.Audio:
        return '.mp3,.flac,.wav';
      case MetadataCategory.Image:
    ...
    <>
      ...
      <Row className="content-action" style={{marginTop: '32px'}}>
        ...
        <Dragger
          ...
          fileList={metadataList as Array<any>}
          onChange={async info => {
            const file = info.file.originFileObj;
        ...
        >
          <div className="ant-upload-drag-icon">
            <h3 style={{ fontWeight: 700 }}>
              Upload all metadata for each NFT.
            </h3>
          </div>
          {metadataError ? (
            <Text type="danger">{metadataError}</Text>
          ) : (
            <p className="ant-upload-text" style={{ color: '#6d6d6d' }}>
              Drag and drop, or click to browse
            </p>
      ...
      <Row className="content-action" style={{marginTop: '32px'}}>
        <h3>Upload NFT(s) (PNG, JPG, GIF, SVG)</h3>
        <Dragger
          accept=".png,.jpg,.gif,.mp4,.svg"
          style={{ padding: 20, background: 'rgba(255, 255, 255, 0.08)' }}
          multiple
          onRemove={(file) => {
                if (nftsList.length === 1 || file.name === mainFile?.name) {
                    setMainFile(undefined)
                }

                setNftsList(nftsList.filter(fi => fi.name !== file.name));
          }}
          customRequest={info => {
            // dont upload files here, handled outside of the control
            info?.onSuccess?.({}, null as any);
          }}
          fileList={nftsList as Array<any>}
          onChange={async info => {
            const file = info.file.originFileObj;
            ...
            setMainFile(file);
            setNftsList([...nftsList, file]);
            setNftsError(undefined);
          }}
        >
          <div className="ant-upload-drag-icon">
            <h3 style={{ fontWeight: 700 }}>
              Upload your NFT(s) (PNG, JPG, GIF, SVG)
            </h3>
          </div>
          {nftsError ? (
            <Text type="danger">{nftsError}</Text>
          ) : (
            <p className="ant-upload-text" style={{ color: '#6d6d6d' }}>
              Drag and drop, or click to browse
            </p>
